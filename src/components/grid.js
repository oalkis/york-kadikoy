...
import { GridItem, GridItemForMenu } from "./gridItem.js";
  ...
  if (g.useInfiniteScroll && g["page" + currentPage]) {
    for (var pageNum = currentPage; ; pageNum++) {
      ...
      if (g[key]) {
        ...
        const lastFetchedPage = Math.min(
          g.cursor,
          props.pageContext.countPages
        );
    ...
    props.pageContext.pageImages.forEach((item) =>
      items.push(<GridItem g={g} item={item} key={"gi" + i++} />)
    );
  }
...
export const GridForMenu = (props) => {
  const g = props.globalState;
  const currentPage = props.pageContext.currentPage;
  const items = [];
  var i = 0,
  ...
  if (g.useInfiniteScroll && g["page" + currentPage]) {
     ...
     */
    props.pageContext.pageImages.forEach((item) =>
      items.push(<GridItemForMenu g={g} item={item} key={"gi" + i++} />)
    );
  }
